...
wss.on("connection", (socket) => {
  ...
  socket.on("message", (message) => {
    ...
    try {
      ...
      writeToFile(
        "logs",
        "websocket_last_request.txt",
        getFormattedWSLog(realRequest, mockRequest)
      );

      if (!mockRequest) return;

      const { mockResponse, useFile, chunkLength, limit, stopAll, delay } =
        mockRequest;
      ...
      if (useFile) {
        ...
        responseData = Array.isArray(mockResponse)
          ? mockResponse
          : [mockResponse];
      }
